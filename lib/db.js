...
export async function getClientGeoMetrics(clientId) {
    ...
    const [
        ...
        supa.from('query_runs').select('id, provider, model, target_found, tracked_query_id, created_at, parse_confidence, parse_status, status').eq('client_id', clientId).eq('status', 'completed').or('run_mode.is.null,run_mode.eq.standard'),

...
/**
 * Agrégations réelles pour le dashboard GEO.
 * Séparation stricte: competitor ≠ generic_mention.
 */
export async function getClientGeoMetrics(clientId) {
    ...
    const [
        ...
        supa.from('query_runs').select('id, provider, model, target_found').eq('client_id', clientId).eq('status', 'completed').or('run_mode.is.null,run_mode.eq.standard'),
    ...
    if (runIds.length > 0) {
        const { data: m } = await supa
            .from('query_mentions')
            .select('entity_type, is_target, business_name, source_type, created_at, query_run_id')
            .in('query_run_id', runIds);
        mentionRows = m || [];
    }

    let confirmedCompetitorMentions = 0;
    let genericMentions = 0;
    let sourceMentions = 0;
    let externalSourceMentions = 0;
    ...
    for (const m of mentionRows) {
        ...
        if (et === 'source') {
            sourceMentions += 1;
            if (m.source_type !== 'client_own') externalSourceMentions += 1;
            if (m.query_run_id) runsWithSourceCitation.add(m.query_run_id);
            if (m.created_at) {
            ...
            confirmedCompetitorMentions += 1;
        } else if (et === 'generic_mention') {
            genericMentions += 1;
        } else if (et === 'business' && m.is_target === false) {
            genericMentions += 1;
        }
        if (m.is_target === true) {
    ...
    const avgSourceMentionsPerRun = tq > 0 ? Math.round((externalSourceMentions / tq) * 10) / 10 : null;
    const runsWithSourceCitationCount = runsWithSourceCitation.size;
    const citationCoveragePercent = tq > 0 ? Math.round((runsWithSourceCitationCount / tq) * 100) : null;
    ...
    try {
        ...
        for (const r of completedRuns || []) {
            ...
            if (!agg.has(key)) {
                agg.set(key, { provider: r.provider || 'unknown', model: r.model || 'unknown', runs: 0, targetFound: 0, sources: 0 });
            }
    ...
    return {
        ...
        confirmedCompetitorMentions,
        genericMentions,
        competitorMentions: confirmedCompetitorMentions,
        sourceMentions,
        externalSourceMentions,
        ...
        visibilityProxyReliability: tq >= 10 ? 'reliable' : tq >= 5 ? 'indicative' : tq > 0 ? 'insufficient_data' : null,

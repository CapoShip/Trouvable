...
import { prepareTrackedQueryWrite } from './operator-intelligence/prompt-taxonomy.js';
...
export async function getLastRunPerTrackedQuery(clientId) {
    const { data: runs, error } = await sb()
        .from('query_runs')
        .select(`id, tracked_query_id, query_text, provider, model, status, target_found, created_at, parsed_response, ${QUERY_RUN_TRACKED_QUERY_SELECT}`)
        .eq('client_id', clientId)
        .order('created_at', { ascending: false });

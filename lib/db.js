...
/**
 * Agrégations GEO dashboard.
 * Delegates to kpi-core pure functions, keeps backward-compatible shape.
 */
export async function getClientGeoMetrics(clientId) {
    ...
    } = await import('@/lib/operator-intelligence/kpi-core');
    ...
    const [
        ...
        supa.from('query_runs').select('id, provider, model, target_found, tracked_query_id, created_at').eq('client_id', clientId).eq('status', 'completed').or('run_mode.is.null,run_mode.eq.standard'),
        ...
        getTrackedQueriesAll(clientId).catch(() => []),
    ]);
    ...
    if (runIds.length > 0) {
        const { data: m } = await supa
            .from('query_mentions')
            .select('entity_type, is_target, business_name, normalized_label, normalized_domain, source_type, created_at, query_run_id')
    ...
    const audit = deriveAuditMetrics(latestAudit);
    const runs = deriveRunMetrics(completedRuns || [], counts);
    const mentions = deriveMentionMetrics(mentionRows, completedRuns || []);
    const prompts = derivePromptMetrics(trackedQueries || [], lastRunMap);

    const snapshot = buildGeoKpiSnapshot({
        audit, runs, mentions, prompts, counts,
        lastRunAt: lastRunRow?.created_at ?? null,
    });

    const legacy = flattenSnapshotToLegacy(snapshot, latestAudit);
    legacy.modelPerformance = enrichModelPerformanceWithSources(
        runs.modelPerformance, mentionRows, completedRuns || [],
    );

...
export async function generateRemediationSuggestionsForClient(clientId) {
    ...
    const [recentRuns, trendSlice, existingSuggestions] = await Promise.all([
    ...
    const notAlreadyDrafted = deduped.filter((problem) => !existingDraftProblemIds.has(problem.id));
    ...
    return {
        ...
        skippedExistingDraft: deduped.length - notAlreadyDrafted.length,

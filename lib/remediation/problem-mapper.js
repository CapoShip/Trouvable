...
export function problemsFromAuditResult(auditResult, clientId) {
    ...
    const llmsTxtIssue = issues.find((issue) =>
        String(issue?.title || '').toLowerCase().includes('llms.txt')
        && String(issue?.title || '').toLowerCase().includes('no ')
    );
    ...
    const crawlerBlockedIssue = issues.find((issue) =>
        String(issue?.title || '').toLowerCase().includes('ai crawler')
        && String(issue?.title || '').toLowerCase().includes('blocked')
    );

...
export async function runGa4SyncForClient(clientId, options = {}) {
    ...
    try {
        [trafficRows, pageRows] = await Promise.all([
            fetchGa4TrafficDaily({ propertyId, startDate: window.startDate, endDate: window.endDate }),
            fetchGa4TopPages({ propertyId, startDate: window.startDate, endDate: window.endDate, limit: 50 }),
        ]);
    } catch (fetchError) {

...
function normalizeSiteUrl(client) {
    let siteUrl = String(client?.website_url || '').trim();
    if (!siteUrl) return null;

    // Si on l'a explicitement configuré en tant que propriété de domaine, on le laisse.
    if (siteUrl.startsWith('sc-domain:')) {
        return siteUrl;
    }

    try {
        // L'API Google Search Console est très stricte avec les "URL Prefix".
        // Le constructeur URL va automatiquement ajouter un '/' à la fin 
        // si c'est simplement une racine d'URL (ex: https://www.trouvable.app -> https://www.trouvable.app/).
        const urlObj = new URL(siteUrl);
        return urlObj.href;
    } catch (e) {
        // Fallback s'il ne s'agit pas d'une URL standard (on retourne tel quel).
        return siteUrl;
    }
}

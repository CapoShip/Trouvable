...
function normalizePhoneCandidate(phone) {
    ...
    const baseValue = extensionMatch
        ? withoutScheme.slice(0, extensionMatch.index).trim()
        : withoutScheme;
    ...
    return withoutScheme.replace(/\s+/g, ' ').trim();
}
...
function extractPhonesAndEmails($) {
    ...
    $('a[href^="mailto:"]').each((_, el) => {
        const mail = normalizeEmailCandidate($(el).attr('href')?.replace('mailto:', '').split('?')[0]);
    ...
    if (emailMatches) {
        emailMatches
            .map((match) => normalizeEmailCandidate(match))
            .filter(Boolean)
            .forEach((match) => emails.add(match));
    }
    ...
    if (phoneMatches.length > 0) {
        phoneMatches
            ...
            .filter(Boolean)
            .forEach((match) => phones.add(match));
    }

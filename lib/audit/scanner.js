...
async function fetchWithTimeout(url) {
    ...
    try {
        const response = await fetch(url, {
            ...
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, TrouvableAuditBot/2.5)',
...
export async function runSiteAudit(startUrl) {
    ...
    while (visitedPages.size < MAX_PAGES_TO_SCAN && pagesToVisit.size > 0) {
        ...
        try {
            ...
            results.extracted_data.technology_signals.has_next_data ||= html.includes('__NEXT_DATA__');
            if (html.includes('__NEXT_DATA__') || html.includes('data-reactroot') || html.includes('id="__nuxt"')) {

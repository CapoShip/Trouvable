...
export function generateMergeSuggestions({ clientId, auditId, client, scanResults, aiAnalysis, automationData }) {
    ...
    const existing = {
        ...
        email: contactInfo.public_email || contactInfo.email || '',
    ...
    for (const auto of automationData || []) {
        ...
        if (auto.field_key === 'public_email' && auto.detected_value) {
            suggest('email', auto.detected_value, auto.confidence_level, 'Email détecté par crawl', 'observed');
        }
        if (auto.field_key === 'short_desc' && auto.detected_value) {
            suggest('short_description', auto.detected_value, 'medium', 'Meta description du site', 'observed');
        }

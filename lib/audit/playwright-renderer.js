...
async function launchServerlessChromium(chromium) {
    try {
        ...
        const browser = await chromium.launch({
            executablePath,
            headless: chromiumBinary.default.headless,
...
export async function createPlaywrightRenderer() {
    ...
    try {
        ...
        const localConnection = (remoteConnection || serverlessConnection) ? null : await launchLocalBrowser(chromium);

        const connection = remoteConnection || serverlessConnection || localConnection;
        if (!connection?.browser) {
            throw new Error('No browser connection strategy available');
        }

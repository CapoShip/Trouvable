 ...
 *   3. Score deterministically (score.js) → 5 dimensions, issues, strengths
 *   4. Extract compact payload for LLM (extract.js)
 *   5. LLM analysis (optional, fallback-safe)
 *   6. Hybrid score = deterministic * 0.85 + LLM * 0.15
 *   7. Persist audit row
...
export async function runFullAudit(clientId, websiteUrl) {
    ...
    try {
        ...
        console.log(`[Audit] Crawl done: ${successPages}/${scanResults.scanned_pages.length} pages OK`);
        ...
        try {
            ...
            llmStatus = 'available';
            console.log(`[Audit] LLM OK via ${aiResult.provider} (comprehension: ${llmScore}/15)`);
        } catch (aiErr) {
            llmStatus = 'failed';
            console.error(`[Audit] LLM failed: ${aiErr.message} — continuing with deterministic-only scoring`);
        }
        ...
        console.log(`[Audit] Done audit=${auditRecord.id} seo=${deterministicScores.seo_score} geo=${deterministicScores.geo_score} overall=${overall_score} llm=${llmStatus} ${elapsedMs}ms`);

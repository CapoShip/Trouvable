    ...
    if (layeredEnabled) {
      try {
        ...
        if (!shadowMode) {
          ...
          // Shadow mode keeps the canonical object out of the persisted
          // extracted_data and only surfaces it in the action log, so the
          // legacy schema contract stays untouched while we benchmark.
          console.log('[Audit] shadow_mode_layered_audit_ready', {
            audit_id: auditRecord.id,
            layer1_overall: layeredAudit?.site_level_raw_scores?.overall ?? null,
            layer2_summary: layeredAudit?.subsystem_scores?.layer2_expert_summary?.summary_score ?? null,
          });
        }
      } catch (buildError) {

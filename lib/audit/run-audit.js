 ...
 *   7. Archive/create opportunities + merge suggestions
 *   8. Final persist audit status
    ...
    if (sharedCtx.auditId && typeof sharedCtx.finalizeAuditOnce === 'function' && !sharedCtx.finalized) {
      try {
        await sharedCtx.finalizeAuditOnce({
          scan_status: 'failed',
          error_message: 'Audit timed out after 90 seconds (global timeout)',
        });
      } catch (finalizeError) {
...
async function _runFullAuditCore(clientId, websiteUrl, startMs, stepTimings, signal, sharedCtx) {
  ...
  function stopBecauseFinalized() {
    return signal?.aborted || sharedCtx.finalized || sharedCtx.finalStatus === 'failed';
  }
  ...
  try {
    ...
    if (!resolvedWebsiteUrl || (!resolvedWebsiteUrl.startsWith('http://') && !resolvedWebsiteUrl.startsWith('https://'))) {
    ...
    if (scanResults.error_message && successPages === 0) {
      ...
      return {
        success: false,
        error: scanResults.error_message,
    ...
    // --- Step 7: Opportunities + merge suggestions (guarded) ---
    t0 = Date.now();
    ...
    try {
      ...
      // --- Step 8: Final persist audit status (LAST awaited terminal write) ---
      t0 = Date.now();

...
function applicabilityMultiplier(level) {
    return APPLICABILITY_MULTIPLIER[level] || APPLICABILITY_MULTIPLIER.medium;
}
...
function summarizeDimension(key, indicators, applicabilityLevel) {
    ...
    if (score >= 80) summary = `${meta.label} is a clear strength based on observed evidence.`;
...
function buildDimensionParts(scanResults, metrics, siteClassification) {
    ...
    const technical = [
        ...
        makeIndicator({ key: 'indexability', label: 'Indexability', max: 14, score: metrics.hasNoindex ? 0 : 14, evidence: metrics.hasNoindex ? 'Noindex observed.' : 'No noindex observed.', status: metrics.hasNoindex ? 'detected' : 'detected' }),
        ...
        makeIndicator({ key: 'rendered_content', label: 'Rendered content', max: 8, score: metrics.hasHydrationRisk ? 2 : metrics.totalWordCount >= 700 ? 8 : metrics.totalWordCount >= 350 ? 5 : 2, evidence: metrics.hasHydrationRisk ? `${metrics.appShellPages} app-shell-like page(s) observed.` : `${metrics.totalWordCount} visible words extracted.`, status: metrics.hasHydrationRisk ? 'weak_evidence' : metrics.totalWordCount >= 350 ? 'detected' : 'weak_evidence' }),
    ];
    ...
    const local = [
        ...
        makeIndicator({ key: 'public_contact', label: 'Public contact', max: 18, applicability: publicContactApplicability, score: metrics.phones.length && metrics.emails.length ? 18 * applicabilityMultiplier(publicContactApplicability) : (metrics.phones.length || metrics.emails.length) ? 10 * applicabilityMultiplier(publicContactApplicability) : 0, evidence: summarize([...metrics.phones, ...metrics.emails], 3) || 'No public phone or email observed.', status: metrics.phones.length && metrics.emails.length ? 'detected' : metrics.phones.length || metrics.emails.length ? 'weak_evidence' : 'not_found' }),
        makeIndicator({ key: 'local_footprint', label: 'Local footprint', max: 26, applicability: serviceAreaApplicability, score: metrics.localEvidence.length >= 5 ? 26 * applicabilityMultiplier(serviceAreaApplicability) : metrics.localEvidence.length >= 2 ? 16 * applicabilityMultiplier(serviceAreaApplicability) : 0, evidence: summarize(metrics.localEvidence, 5) || 'No strong local footprint observed.', status: metrics.localEvidence.length >= 5 ? 'detected' : metrics.localEvidence.length >= 2 ? 'weak_evidence' : serviceAreaApplicability === 'low' ? 'not_applicable' : 'not_found' }),
        makeIndicator({ key: 'supporting_pages', label: 'Local support pages', max: 12, applicability: serviceAreaApplicability, score: metrics.localPageCount > 0 ? 12 * applicabilityMultiplier(serviceAreaApplicability) : metrics.servicePageCount > 0 ? 6 * applicabilityMultiplier(serviceAreaApplicability) : 0, evidence: metrics.localPageCount > 0 ? `${metrics.localPageCount} local page(s) observed.` : metrics.servicePageCount > 0 ? `${metrics.servicePageCount} service page(s) observed, but no local page.` : 'No local support page observed.', status: metrics.localPageCount > 0 ? 'detected' : metrics.servicePageCount > 0 ? 'weak_evidence' : serviceAreaApplicability === 'low' ? 'not_applicable' : 'not_found' }),
    ...
    const answerability = [
        ...
        makeIndicator({ key: 'structure', label: 'Heading structure', max: 12, score: metrics.h2Rich ? 12 : metrics.h1.length >= 8 ? 6 : 0, evidence: metrics.h2Rich ? 'At least one strong H2 cluster observed.' : metrics.h1 || 'Very little heading structure observed.', status: metrics.h2Rich ? 'detected' : metrics.h1.length >= 8 ? 'weak_evidence' : 'weak_evidence' }),
        makeIndicator({ key: 'extractable_identity', label: 'Extractable identity cues', max: 20, score: metrics.businessNames.length && metrics.schemaEntities.length ? 20 : metrics.businessNames.length || metrics.schemaEntities.length ? 11 : 3, evidence: summarize(metrics.businessNames, 2) || (metrics.schemaEntities.length ? `${metrics.schemaEntities.length} schema entities observed.` : 'Few identity cues observed.'), status: metrics.businessNames.length && metrics.schemaEntities.length ? 'detected' : metrics.businessNames.length || metrics.schemaEntities.length ? 'weak_evidence' : 'weak_evidence' }),
    ];
    ...
    const identity = [
        ...
        makeIndicator({ key: 'business_context', label: 'Business context and services', max: 22, score: metrics.serviceSignals.services?.length >= 3 ? 22 : metrics.serviceSignals.services?.length || metrics.servicePageCount ? 14 : 6, evidence: summarize(metrics.serviceSignals.services || metrics.serviceEvidence, 5) || 'Limited service detail observed.', status: metrics.serviceSignals.services?.length >= 3 ? 'detected' : metrics.serviceSignals.services?.length || metrics.servicePageCount ? 'weak_evidence' : 'weak_evidence' }),
...
export function scoreAuditV2(scanResults, providedSiteClassification = null) {
    ...
    return {
        ...
        issues: sortIssues(parts.issues),

...
export async function deferRunForOverlap(runId) {
    const { error } = await db()
        .from('recurring_job_runs')
        .update({
            scheduled_for: addMinutes(getDbNowIso(), 5),
            error_message: 'Deferred to prevent overlap for same client/job_type.',
        })
        .eq('id', runId)
        .eq('status', 'pending');

    if (error) throw new Error(`[DB/jobs] deferRunForOverlap ${runId}: ${error.message}`);
}
...
export async function claimPendingRun(runId, nextAttempt) {
    ...
    if (error) throw new Error(`[DB/jobs] claimPendingRun ${runId}: ${error.message}`);
    return data || null;
}

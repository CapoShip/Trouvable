...
export async function deleteProblematicQueryRuns(clientId) {
    // Delete failed runs (status = 'failed')
    const { error: failedError, count: failedCount } = await db()
        .from('query_runs')
        .delete({ count: 'exact' })
        .eq('client_id', clientId)
        .eq('status', 'failed');

    if (failedError) throw new Error(`[DB] deleteProblematicQueryRuns (failed): ${failedError.message}`);

    // Delete runs with parse failures (parsed_failed or parsed_partial) that are not already deleted
    const { error: parseError, count: parseCount } = await db()
        .from('query_runs')
        .delete({ count: 'exact' })
        .eq('client_id', clientId)
        .in('parse_status', ['parsed_failed', 'parsed_partial']);

    if (parseError) throw new Error(`[DB] deleteProblematicQueryRuns (parse): ${parseError.message}`);

    return { deleted: (failedCount || 0) + (parseCount || 0) };
}

...
export async function getTrendSlice(clientId) {
    ...
    const [ws, jobHealth, snapshotsResult, connectors] = await Promise.all([
...
export async function processContinuousWorkerTick(rawOptions = {}) {
    ...
    for (const runnable of runnableRuns) {
        try {
            ...
            summary.failed += 1;
            summary.errors.push(`run:${runnable?.id || 'unknown'} ${runError.message}`);
            console.error('[Continuous] processContinuousWorkerTick run error:', runError);
        }
    }

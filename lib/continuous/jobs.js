        ...
        if (!failError) {
            ...
            await supabase
                ...
                .update({
                    ...
                    last_failure_at: nowIso(),
                    last_run_at: nowIso(),
                    next_run_at: addMinutes(nowIso(), Number(job?.cadence_minutes || 1440)),
                })
                .eq('id', run.job_id);
        }
    }
...
async function finalizeRunSuccess({ run, job, summary }) {
    ...
    const nextRunAt = addMinutes(now, Number(job.cadence_minutes || 1440));
...
export async function updateRecurringJobCadence({ jobId, cadenceMinutes, retryLimit, retryBackoffMinutes }) {
    const supabase = getAdminSupabase();
    const payload = {
        cadence_minutes: clamp(Number(cadenceMinutes), 15, 10080),
...
export async function getTrendSlice(clientId) {
    ...
    const metricDefinitions = [
        ...
        { key: 'geo_score', label: 'GEO score' },
        { key: 'visibility_proxy_percent', label: 'Visibility proxy' },
        { key: 'mention_rate_percent', label: 'Prompt mention rate' },
        { key: 'citation_coverage_percent', label: 'Citation coverage' },
        { key: 'competitor_visibility_count', label: 'Competitor visibility' },
    ];
    ...
    const auditFreshness = classifyFreshness(metrics?.lastAuditAt, 96);
    const runFreshness = classifyFreshness(metrics?.lastGeoRunAt, 48);
    ...
    for (const metric of board.declining) {
        if (metric.key === 'seo_score' || metric.key === 'geo_score') {
            actionCenter.push({
                ...
                priority: 'high',
                title: `${metric.label} dropped (${metric.delta})`,
                rationale: 'Recent trend indicates declining audit score. Re-run audit and prioritize unresolved issues.',
        ...
        if (metric.key === 'citation_coverage_percent') {
            actionCenter.push({
                ...
                priority: 'high',
                title: 'Citation coverage declining',
                rationale: 'Observed source coverage fell in recent snapshots. Expand citation and source capture opportunities.',
                evidence: 'derived_from_snapshots',
            });
        }
        ...
        if (metric.key === 'mention_rate_percent') {
            actionCenter.push({
                ...
                priority: 'medium',
                title: 'Prompt mention rate declining',
                rationale: 'Tracked prompt visibility is weakening. Refresh prompt set and run cadence.',
                evidence: 'derived_from_snapshots',
            });
        }
    }
    ...
    if ((metrics?.trackedPromptStats?.noRunYet || 0) > 0) {
        actionCenter.push({
            ...
            category: 'prompt_coverage_gaps',
            priority: 'medium',
            title: 'Some tracked prompts still have no run',
            rationale: `${metrics.trackedPromptStats.noRunYet} prompt(s) are missing first-run observations.`,
    ...
    if (auditFreshness.state === 'stale') {
        actionCenter.push({
            ...
            priority: 'high',
            title: 'Audit freshness is stale',
            rationale: `Latest audit is ${auditFreshness.hours}h old. Trigger an audit refresh.`,
    ...
    if (runFreshness.state === 'stale') {
        actionCenter.push({
            ...
            category: 'freshness_rerun_issues',
            priority: 'high',
            title: 'Tracked runs freshness is stale',
            rationale: `Latest tracked run is ${runFreshness.hours}h old. Trigger scheduled prompt reruns.`,
            evidence: 'observed_timestamp',
        });
    }
    ...
    if ((metrics?.competitorMentions || 0) > Math.max(10, (metrics?.brandRecommendationRuns || 0))) {
        actionCenter.push({
            ...
            priority: 'medium',
            title: 'Competitor pressure is elevated',
            rationale: 'Observed competitor mentions are high relative to brand recommendation runs.',

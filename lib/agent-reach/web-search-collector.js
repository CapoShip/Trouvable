    ...
    try {
        const response = await fetch('https://api.tavily.com/search', {
            ...
            body: JSON.stringify({
                ...
                search_depth: 'basic',
                include_domains: ['reddit.com', 'quora.com', 'trustpilot.com', 'avis-verifies.com', 'glassdoor.com', 'indeed.com', 'facebook.com', 'lesjeudis.com'],
            }),
        });
        if (!response.ok) {
...
async function searchWeb({ query, maxResults = WEB_SEARCH_MAX_RESULTS_PER_QUERY }) {
    ...
    try {
        ...
        console.warn('[Community/WebSearch] Tavily search failed:', err?.message);
    }
    ...
    try {
        ...
        console.warn('[Community/WebSearch] Google CSE search failed:', err?.message);
    }
...
function detectSourceFromUrl(url) {
    ...
    if (hostname.includes('avis-verifies.com')) return 'avis_verifies';
    return 'web';
}
...
export async function collectViaWebSearch(seedQueries) {
    ...
    for (const post of allResults) {
        const key = post.permalink || `${post.title}:${post.body?.slice(0, 100)}`;
        if (!dedupe.has(key)) {
            dedupe.set(key, post);
        }
    }

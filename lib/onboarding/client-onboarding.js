...
function deriveBusinessType(siteTypeObj, fallback = '', clientName = '') {
    const rawType = String(fallback || '').trim();
    const resolved = resolveBusinessType(rawType, siteTypeObj || { type: 'generic_business' }, clientName);
    
    // Return canonical category or a fallback if still empty
    if (resolved.needs_review) {
        return ''; // Return empty string so the operator explicitly reviews and enters a real category
    }
    return resolved.canonical_category || 'service_local';
}
...
function buildProfileSuggestion({
    ...
    const detectedBusinessType = deriveBusinessType(classification, input.category, businessNames[0] || input.business_name);
...
function buildSuggestionSignals({
    ...
    const resolved = resolveBusinessType(rawCategory, classification, businessNames[0] || input.business_name);
    ...
    return {
        ...
        resolved_category: {
            raw_schema_input: rawCategory,
            business_model: resolved.business_model_detected,
            canonical_category: resolved.canonical_category,
            confidence: resolved.category_confidence,
            needs_review: resolved.needs_review,
            reason: resolved.category_resolution_reason,
        },
        social: {
...
export async function startClientOnboarding(rawInput, options = {}) {
    ...
    const suggestionClient = {
        client_name: businessNames[0] || input.business_name,
        business_type: deriveBusinessType(classification, input.category, businessNames[0] || input.business_name),
        address: { city: input.primary_region },
        target_region: input.primary_region,
    };
...
export async function activateClientOnboarding(rawInput, options = {}) {
    ...
    const desiredBusinessType = String(profile.business_type || client.business_type || 'LocalBusiness').trim() || 'LocalBusiness';

...
function deriveBusinessType(siteType, fallback = 'LocalBusiness') {
    if (siteType === 'saas_software') return 'SoftwareApplication';
    if (siteType === 'content_led') return 'Organization';
    if (siteType === 'local_business') return 'LocalBusiness';
    return String(fallback || 'LocalBusiness').trim() || 'LocalBusiness';
}
...
function buildProfileSuggestion({
    ...
    const detectedBusinessType = deriveBusinessType(classification?.type, input.category);
...
export async function startClientOnboarding(rawInput, options = {}) {
    const input = {
        ...
        category: String(rawInput?.category || 'LocalBusiness').trim() || 'LocalBusiness',
    ...
    const suggestionClient = {
        client_name: businessNames[0] || input.business_name,
        business_type: deriveBusinessType(classification?.type, input.category),

...
describe('app/robots.js — output contract', () => {
    it('contains only standard robots.txt directives', async () => {
        ...
        // Next.js serializes the object to text/plain in production.
        // We reconstruct the text representation here.
        const lines = [];
    ...
    it('does not contain Content-Signal directive', async () => {
        const mod = await import('../../app/robots.js');
        const result = mod.default();

        // Serialize to check raw output
        const serialized = JSON.stringify(result);
        expect(serialized).not.toContain('Content-Signal');
        expect(serialized).not.toContain('content-signal');
        expect(serialized).not.toContain('ai-train');
    });
});

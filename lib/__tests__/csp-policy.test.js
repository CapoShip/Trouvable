...
describe('CSP policy hardening', () => {
    ...
    it('allows Microsoft Clarity tracking endpoints', () => {
        for (const directives of [proxyDir, vercelDir]) {
            expect(directives['default-src']).toContain('https://*.clarity.ms');
            expect(directives['default-src']).toContain('https://c.bing.com');
            expect(directives['script-src']).toContain('https://*.clarity.ms');
            expect(directives['connect-src']).toContain('https://*.clarity.ms');
            expect(directives['connect-src']).toContain('https://c.bing.com');
        }
    });

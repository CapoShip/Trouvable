...
function buildPortalTrendSummary(snapshots = []) {
    const definitions = [
        { key: 'seo_score', label: 'SEO score', unit: 'score' },
        { key: 'geo_score', label: 'GEO score', unit: 'score' },
        { key: 'visibility_proxy_percent', label: 'Visibility proxy', unit: 'percent' },
        { key: 'citation_coverage_percent', label: 'Citation coverage', unit: 'percent' },
        { key: 'mention_rate_percent', label: 'Prompt mention rate', unit: 'percent' },
    ];
...
export async function getPortalDashboardData(clientId) {
    ...
    const [metrics, trackedQueries, lastRunMap, opportunitiesResult, actionsResult, snapshotsResult] = await Promise.all([
        db.getClientGeoMetrics(clientId).catch((error) => {

...
export function resolveBusinessType(businessType, siteClassification = {}, clientName = '') {
    ...
    let canonicalCategory = 'service_local';
    ...
    if (descriptiveLabel) {
        canonicalCategory = slugifyLabel(descriptiveLabel) || 'service_local';
    ...
    const typeHay = `${rawType} ${siteType} ${siteLabel} ${servicesText} ${narrativeHay}`;
    ...
    if (/trouvable/i.test(nameHay) && /(geo|seo|visibilite|ia)/i.test(typeHay)) {
        businessModelDetected = 'saas';

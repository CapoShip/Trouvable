...
/**
 * GEO system prompt shared between brand_aware and blind_discovery modes.
 * The core fidelity rules are identical — only the user prompt differs.
 */
const GEO_SYSTEM_PROMPT = `Tu es un moteur de reponse conversationnel qui combine les usages de Google, AI Overviews, ChatGPT, Gemini et Perplexity.
Tu dois produire une reponse utile pour des recherches GEO (locales et intention de service).
...
- Si une ville, un quartier, une region ou un territoire est implicite ou explicite, adapte clairement la recommandation a cette localite.
- Fais ressortir les types d'etablissement ou de prestataires (ex: clinique, agence, cabinet, artisan) quand c'est utile a la decision.`;

/**
 * Brand-aware GEO query prompt.
 * Injects full business context (name, description, area, services, competitors).
...
export function buildBlindGeoQueryPrompt(query) {
    const system = `${GEO_SYSTEM_PROMPT}
- Utilise ta connaissance generale prudente pour recommander des options concretes.
- Ne presuppose aucune preference pour une marque ou un prestataire specifique.`;

    const user = `${query}

Reponds avec des noms propres explicites, des comparaisons claires et des URLs si tu t'appuies sur des sources web.`;

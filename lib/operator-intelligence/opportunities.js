...
export async function getOpportunitySlice(clientId) {
    ...
    const all = (opportunities || []).map((opportunity) => ({
        ...opportunity,
        provenance: mapOpportunitySourceToProvenance(opportunity.source),
        isLatestAudit: latestAuditId ? opportunity.audit_id === latestAuditId : true,
    }));

    const activeOpportunities = all.filter((o) => o.isLatestAudit || !o.audit_id || o.status !== 'open');
    const staleOpportunities = all.filter((o) => !o.isLatestAudit && o.audit_id && o.status === 'open');

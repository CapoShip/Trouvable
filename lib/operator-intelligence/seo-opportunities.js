    ...
    const items = Array.from(pageMetricsMap.values())
        ...
        .map((page) => {
            ...
            return makeOpportunityItem({
                ...
                why: `Page déjà visible autour de la position ${page.position?.toFixed(1) || '—'} avec un volume exploitable d’impressions.`,
    ...
    const items = Array.from(pageMetricsMap.values())
        ...
        .map((page) => {
            ...
            return makeOpportunityItem({
                ...
                evidence: `${page.impressions.toLocaleString('fr-FR')} impressions, ${page.clicks.toLocaleString('fr-FR')} clics, CTR ${page.ctr === null ? '—' : `${(page.ctr * 100).toFixed(1)}%`} et position moyenne ${page.position?.toFixed(1) || '—'}.`,
    ...
    for (const block of toArray(onPage?.blocks)) {
        ...
        for (const blockItem of toArray(block?.items)) {
            ...
            items.push(makeOpportunityItem({
                ...
                evidence: metrics
                    ? `${toNumber(metrics.impressions).toLocaleString('fr-FR')} impressions et position moyenne ${metrics.position?.toFixed(1) || '—'} sur Search Console.`

...
function buildPromptBlueprints({ clientName, categoryLabel, city, locale, services, knownCompetitors, resolvedBusiness }) {
    ...
    const secondService = String(services[1] || '').trim();
    ...
    const audienceFr = resolvedBusiness?.target_audience === 'b2b' ? 'une PME ou une equipe interne' : 'un particulier';
    const outcomePhrase = secondService && anchor ? `${anchor} (dont ${secondService})` : (anchor || primaryService || `l'offre de ${clientName}`);
...
export function buildStarterPromptPack({ client, siteType, siteClassification, locale, existingPrompts }) {
    ...
    const mergedClassification = {
        ...(siteClassification || {}),
        services_preview: Array.isArray(client?.business_details?.services) ? client.business_details.services : [],
        short_description_preview: getBusinessShortDescription(client?.business_details || {}).slice(0, 400),
        seo_teaser: String(client?.seo_description || '').trim().slice(0, 220),
    };
    const resolvedBusiness = resolveBusinessType(categoryLabel, mergedClassification, clientName);
...
export async function getPromptSlice(clientId) {
    ...
    return {
        ...
        siteContext: {
            ...
            resolved_business: resolveBusinessType(

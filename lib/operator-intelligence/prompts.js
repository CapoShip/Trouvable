...
export async function getPromptSlice(clientId) {
    ...
    const spontaneousTargetFound = blindWithRun.filter((p) => p.last_run.target_found === true).length;
    ...
    const visibilityByMode = {
        blind_discovery: {
            ...
            target_found: spontaneousTargetFound,
            mention_rate_percent: spontaneousTotal > 0 ? Math.round((spontaneousTargetFound / spontaneousTotal) * 100) : null,
            label: 'Visibilite spontanee',
        },
        brand_aware: {
            total_prompts: brandPrompts.length,
            with_run: assistedTotal,
            target_found: assistedTargetFound,
            mention_rate_percent: assistedTotal > 0 ? Math.round((assistedTargetFound / assistedTotal) * 100) : null,
            label: 'Visibilite assistee (marque)',
        },
    };

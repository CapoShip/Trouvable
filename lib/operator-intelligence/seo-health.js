...
export async function getSeoHealthSlice(clientId, { audit } = {}) {
    ...
    // SEO-relevant categories from issue/strength objects
    const seoCategories = new Set(['technical', 'seo', 'trust', 'identity']);
    const filterSeoRelevant = (items) =>
        items.filter((item) => {
            const cat = (item.category || '').toLowerCase();
            return seoCategories.has(cat) || cat.startsWith('technical') || cat.startsWith('seo') || cat.startsWith('identity');
        });

 ...
 * - audit_issues from latest audit (SEO categories)
 */
export async function getSeoActionsSlice(clientId, { audit } = {}) {
    ...
    const seoCategories = new Set(['technical', 'seo', 'trust']);
    const auditIssues = [];
    if (audit?.audit_result?.audit_issues) {
        for (const issue of audit.audit_result.audit_issues) {
            const cat = issue.category || '';
            if (seoCategories.has(cat) || cat.startsWith('technical') || cat.startsWith('seo')) {

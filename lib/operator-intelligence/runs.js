...
export async function getRunsSlice(clientId) {
    ...
    const history = (runs || []).map((run) => {
        ...
        if (parseCounts[run.parse_status] !== undefined) {
            parseCounts[run.parse_status] += 1;
        }
...
export async function getRunInspectorSlice(clientId, runId) {
    ...
    const grouped = buildGroupedMentions(mentions);
    const diagnostics = run.raw_analysis?.diagnostics || {};
    ...
    return {
        ...
        run: {
            ...
            prompt_payload: run.prompt_payload || {},
        ...
        diagnostics: {
            zero_citation_reason: diagnostics.zero_citation_reason || (grouped.sourceMentions.length === 0 ? 'Aucune source explicite detectee dans la reponse brute.' : null),
            zero_competitor_reason: diagnostics.zero_competitor_reason || (grouped.competitorMentions.length === 0 ? 'Aucun concurrent fiable detecte.' : null),
        },
    };
}

...
export async function getOverviewSlice(clientId) {
    ...
    const [
        ...
        supa.from('query_runs').select('id, provider, model, target_found, tracked_query_id, created_at, query_text').eq('client_id', clientId).eq('status', 'completed').or('run_mode.is.null,run_mode.eq.standard'),
        ...
        supa.from('opportunities').select('id, title, description, priority, category, status, source, created_at, audit_id')
    ...
    const relevantOpps = latestAuditId
        ? (openOpportunities || []).filter((o) => o.audit_id === latestAuditId || !o.audit_id)
        : (openOpportunities || []);
    ...
    return {
        ...
        kpis: {
            ...
            openOpportunitiesCount: counts.openOpportunities,

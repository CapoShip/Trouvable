...
export async function getSeoLocalSlice(clientId, { audit } = {}) {
    ...
    // Local-relevant categories
    const localCategories = new Set(['geo', 'local', 'content']);
    const localIssues = issues.filter((item) => {
        const cat = (item.category || '').toLowerCase();
        return localCategories.has(cat) || cat.startsWith('local') || cat.startsWith('geo');
    });

...
export default function ClientForm({ initialData = null }) {
    ...
    const handleSubmit = (e) => {
        ...
        const payload = {
            ...formData,
            // Filter out empty social profiles to avoid array pollution
            social_profiles: formData.social_profiles.filter(p => p.trim() !== ''),
            // Filter out empty FAQs
            geo_faqs: formData.geo_faqs.filter(f => f.question.trim() !== '' && f.answer.trim() !== '')
        };

        startTransition(async () => {
            const res = await saveClientProfileAction(payload);
            if (res?.error) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-ea580c focus:border-ea580c outline-none"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={handleChange}
                                className="w-full px-4 py-2 font-mono text-sm border border-slate-300 rounded-lg focus:ring-ea580c focus:border-ea580c outline-none bg-slate-50"
                            />
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <input
                                ...
                                className="w-full px-3 py-2 text-sm border border-slate-300 rounded focus:ring-ea580c focus:border-ea580c outline-none"
                            />
                            <input
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button type="button" onClick={addSocialProfile} className="text-ea580c text-sm font-semibold hover:underline border border-ea580c px-2 py-0.5 rounded transition-colors hover:bg-orange-50">+ Ajouter</button>
                            </div>
                                ...
                                {formData.social_profiles.map((sp, i) => (
                                    <div key={i} className="flex gap-2">
                                        <input
                                            ...
                                            className="w-full px-3 py-1.5 text-sm border border-slate-300 rounded focus:ring-ea580c focus:border-ea580c outline-none"
                                        />
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button type="button" onClick={addFaq} className="text-ea580c text-sm font-semibold hover:underline border border-ea580c px-2 py-0.5 rounded transition-colors hover:bg-orange-50">+ FAQ</button>
                            </div>
                            <div className="space-y-4">
                                {formData.geo_faqs.length === 0 && <p className="text-xs text-slate-500 italic">Aucune question.</p>}
                                {formData.geo_faqs.map((faq, i) => (
                                    <div key={i} className="relative border-l-2 border-ea580c pl-3">
                                        ...
                                        <div className="space-y-2 pr-4">
                                            <input
                                                ...
                                                className="w-full px-3 py-1.5 text-sm font-medium border border-slate-300 rounded focus:ring-ea580c focus:border-ea580c outline-none focus:bg-white"
                                            />
                                            <textarea
                                                value={faq.answer}
                                                ...
                                                placeholder="Réponse"
                                                rows="2"
                                                className="w-full px-3 py-1.5 text-sm border border-slate-300 rounded focus:ring-ea580c focus:border-ea580c outline-none resize-none focus:bg-white"
                                            ></textarea>
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="md:col-span-2 pt-6 flex items-center justify-between border-t mt-6 border-slate-200">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            onChange={handleChange}
                            className="w-5 h-5 rounded border-slate-300 text-ea580c focus:ring-ea580c"
                        />
                    ...
                    <button
                        type="submit"
                        disabled={isPending}
                        className="px-6 py-3 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800 transition-colors disabled:opacity-50 min-w-[200px]"
                    >

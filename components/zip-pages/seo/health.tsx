    ...
    const chartData = useMemo(() => {
        if (history.length > 0) {
            return history.map((entry) => ({
                label: entry.label,
                value: entry.crawlIssueCount > 0 ? entry.crawlIssueCount : entry.issueCount,
            }));
        }

        return sortedIssues.slice(0, 6).map((issue, index) => ({
            label: `A${index + 1}`,
            value: ['critical', 'high'].includes(String(issue?.priority || '').toLowerCase()) ? 3 : String(issue?.priority || '').toLowerCase() === 'medium' ? 2 : 1,
        }));
    }, [history, sortedIssues]);
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <CommandChartCard title="Évolution des erreurs de crawl" className="h-[300px] lg:col-span-2">
                    {chartData.length > 0 ? (
                        ...
                        <CommandEmptyState
                            ...
                            description="L historique d audits n est pas encore suffisant pour dessiner une courbe fiable."
                        />
                    )}
                </CommandChartCard>

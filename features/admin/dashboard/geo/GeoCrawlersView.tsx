...
export default function GeoCrawlersPage() {
    ...
    const chartRows = (data?.botRows || []).map((item) => ({
        ...
        fill:
            toneFromStatus(item.operatorStatus) === 'ok'
                ? '#10b981'
                : toneFromStatus(item.operatorStatus) === 'warning'
                    ? '#f59e0b'
                    : toneFromStatus(item.operatorStatus) === 'critical'
                        ? '#f43f5e'
                        : '#8b5cf6',
    }));
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-4 lg:grid-cols-3">
                        <CommandChartCard title="Regles observees par bot" className="lg:col-span-1">
                            {chartRows.length === 0 ? (
                                <div className="flex h-[240px] items-center justify-center text-[12px] text-white/45">Aucune regle exploitable observee.</div>
                            ) : (

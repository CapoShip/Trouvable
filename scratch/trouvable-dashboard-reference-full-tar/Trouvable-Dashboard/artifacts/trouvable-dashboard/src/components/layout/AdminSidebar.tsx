...
const PORTFOLIO_NAV = [
  ...
  { id: 'clients', label: 'Portefeuille', icon: 'portfolio', path: '/clients' },
  { id: 'new', label: 'Nouveau mandat', icon: 'plus', path: '/clients/new' },
];
...
function NavItem({ href, active, icon, children }: { href: string; active: boolean; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <Link href={href} className={`group relative flex items-center gap-2.5 px-3 py-[7px] mx-2 rounded-lg transition-all duration-200 text-[12px] font-medium ${active ? 'bg-white/[0.06] text-white' : 'text-white/40 hover:bg-white/[0.03] hover:text-white/65'}`}>
      {active && (
        <motion.div
...
export function AdminSidebar() {
  ...
  // Always assume a demo client context for this build
  const clientBase = '/clients/demo';
  const clientId = 'demo';

  const isDashboard = location === '/';
  const isClientList = location === '/clients';
  const isNewClient = location === '/clients/new';

  const activePath = location.replace(clientBase, '') || '/';
      ...
      <nav className={`geo-sb ${mobileOpen ? 'open' : ''}`}>
        <div className="px-4 py-4 border-b border-white/[0.05]">
          <div className="flex items-center justify-between gap-2">
            <Link href="/" className="flex items-center gap-2.5 rounded-lg -mx-1 px-1 py-0.5 transition-colors hover:bg-white/[0.03] min-w-0">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#5b73ff]/20 to-[#8b5cf6]/10 border border-[#5b73ff]/15 flex items-center justify-center shrink-0">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4 text-[#7b8fff] stroke-linecap-round stroke-linejoin-round">
          ...
          <div className="py-2 pb-6">
            <NavGroup label="Supervision">
              {PORTFOLIO_NAV.map((item) => (
                <NavItem key={item.id} href={item.path} active={location === item.path} icon={ICONS[item.icon]}>
                  {item.label}
                </NavItem>
              ))}
            </NavGroup>
                ...
                >
                  <NavGroup label="Dossier partagé">
                    {CLIENT_DOSSIER_NAV.map((item) => (
                      <NavItem key={item.id} href={`${clientBase}${item.path}`} active={activePath === item.path} icon={ICONS[item.icon]}>
                        {item.label}
                      </NavItem>
                    ))}
                  </NavGroup>
